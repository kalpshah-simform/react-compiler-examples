...
/**
 * Never touches the store — only ever receives `label`, which its parent
 * passes as the same value on every render. Lets us compare its render count
 * against a sibling that subscribes: if this stays flat while the sibling
 * climbs, the re-render came from the store subscription, not from the
 * parent re-rendering.
 */
